...
export class SearchResultComponent implements OnDestroy, AfterViewInit {
  ...
  trustProductDescription (tableData: any[]) { // vuln-code-snippet neutral-line restfulXssChallenge
    for (let i = 0; i < tableData.length; i++) { // vuln-code-snippet neutral-line restfulXssChallenge
      // Using Angular's built-in sanitization instead of bypassing it
      tableData[i].description = tableData[i].description // vuln-code-snippet vuln-line restfulXssChallenge
  ...
  filterTable () {
    ...
    if (queryParam) {
      ...
      this.searchValue = queryParam // Using Angular's built-in sanitization instead of bypassing it // vuln-code-snippet vuln-line localXssChallenge xssBonusChallenge
