...
exports.checkVulnLines = () => async (req: Request<Record<string, unknown>, Record<string, unknown>, VerdictRequestBody>, res: Response, next: NextFunction) => {
  ...
  let hint
  const sanitizedKey = security.sanitizeFilename(key)
  const codingChallengeInfoPath = path.resolve('data/static/codefixes', sanitizedKey + '.info.yml')
  if (fs.existsSync(codingChallengeInfoPath)) {
    const codingChallengeInfos = yaml.load(fs.readFileSync(codingChallengeInfoPath, 'utf8'))
