    ...
    BasketModel.findOne({ where: { id }, include: [{ model: ProductModel, paranoid: false, as: 'Products' }] })
      .then(async (basket: BasketModel | null) => {
        if (basket != null) {
          ...
          let sanitizedPaymentId = null
          let sanitizedAddressId = null

          if (req.body.orderDetails) {
            // Convert to string and sanitize if exists, otherwise use null
            sanitizedPaymentId = req.body.orderDetails.paymentId ? String(req.body.orderDetails.paymentId).replace(/[^\w.-]/g, '') : null
            sanitizedAddressId = req.body.orderDetails.addressId ? String(req.body.orderDetails.addressId).replace(/[^\w.-]/g, '') : null
          }
