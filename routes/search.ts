...
module.exports = function searchProducts () {
  return (req: Request, res: Response, next: NextFunction) => {
    ...
    models.sequelize.query(
      `SELECT * FROM Products WHERE ((name LIKE :nameCriteria OR description LIKE :descCriteria) AND deletedAt IS NULL) ORDER BY name`, // vuln-code-snippet vuln-line unionSqlInjectionChallenge dbSchemaChallenge
      { 
        replacements: { 
          nameCriteria: `%${criteria}%`, 
          descCriteria: `%${criteria}%` 
        } 
      })
      .then(([products]: any) => {
