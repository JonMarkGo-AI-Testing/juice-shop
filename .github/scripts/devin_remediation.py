...
GITHUB_TOKEN = os.getenv("GITHUB_TOKEN")  # Using PAT for better permissions
...
async def get_sonarcloud_issues():
    ...
    if not SONAR_TOKEN:
        log("SONAR_TOKEN environment variable is not set", "ERROR")
        return []
    ...
    log(f"Fetching SonarCloud issues with params: {params}")
    
    try:
        async with aiohttp.ClientSession() as session:
            async with session.get(url, headers=headers, params=params) as response:
                if response.status != 200:
                    log(f"Error getting SonarCloud issues: {await response.text()}", "ERROR")
                    return []
                    
                result = await response.json()
                issue_count = len(result.get('issues', []))
                log(f"Found {issue_count} issues")
                return result.get('issues', [])
    except aiohttp.ClientError as e:
        log(f"Network error when connecting to SonarCloud: {str(e)}", "ERROR")
        return []
    except Exception as e:
        log(f"Unexpected error when fetching SonarCloud issues: {str(e)}", "ERROR")
        return []
